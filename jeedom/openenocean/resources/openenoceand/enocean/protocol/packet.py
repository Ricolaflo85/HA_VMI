...
class Packet(object):
    ...
    def __init__(self, packet_type, data=None, optional=None):
        ...
        if not isinstance(data, list) or data is None:
            logging.debug('Replacing Packet.data with default value.')
            self.data = []
        else:
            self.data = data

        if not isinstance(optional, list) or optional is None:
            logging.debug('Replacing Packet.optional with default value.')
            self.optional = []
        else:
    ...
    def parse_msg(buf, communicator=None):
        ...
        elif packet_type == PACKET.REMOTE_MAN_COMMAND:
            packet = RemoteCoPacket(packet_type, data, opt_data)
        else:
    ...
    def create(packet_type, rorg, rorg_func, rorg_type, direction=None, command=None,
        ...
        if packet_type != PACKET.RADIO and packet_type != PACKET.RESPONSE:
        ...
        if destination is None:
            logging.debug('Replacing destination with broadcast address.')
        ...
        if sender is None:
            logging.debug('Replacing sender with default address.')
...
class UTETeachIn(RadioPacket):
    ...
    def _create_response_packet(self, sender_id, response=TEACHIN_ACCEPTED):
        ...
        if globals.EXCLUDE_MODE == True :
            logging.info('Exclude mode send DELETE accepted.')
            DELETE_ACCEPTED = [True, False]
    ...
    def send_response(self, response=TEACHIN_ACCEPTED):
        if self.__communicator is None:
            logging.error('Communicator not set, cannot send UTE teach-in response.')
            return
        ...
        if globals.LEARN_MODE == False and globals.EXCLUDE_MODE == False:
            logging.info('Ute but jeedom not in learn or exclude')
            return
        logging.info('Sending response to UTE teach-in.')
        self.__communicator.send(self._create_response_packet(self.__communicator.base_id))
